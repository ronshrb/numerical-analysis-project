...
class Assignment4:
    ...
    def fit(self, f: callable, a: float, b: float, d:int, maxtime: float) -> callable:
        ...
        for i in range(n):
            ...
            b_mat.append(y)
            a_mat += [[np.power(x,d-j) for j in range(d+1)]]
        ...
        def least_squares(A, b):
            # calculations of least squares
            A_t = np.transpose(A)
            AtA = np.dot(A_t, A)
            AtA_inv = np.linalg.inv(AtA)
            AtA_inv_At = np.dot(AtA_inv, A_t)
            c = np.dot(AtA_inv_At, b)
            return c

        result = least_squares(a_mat,b_mat)
        sol = np.poly1d(result)
